...
public class CategoryService
{
    ...
    {
        var result = await _http.GetFromJsonAsync<ApiResponse<List<CategoryResponse>>>("api/categories");
    ...
    {
        var response = await _http.PostAsJsonAsync("api/categories", req);
    ...
    {
        var response = await _http.PutAsJsonAsync($"api/categories/{id}", req);
        var result = await response.Content.ReadFromJsonAsync<ApiResponse<CategoryResponse>>();
        return result ?? new ApiResponse<CategoryResponse>
        {
            Success = false,
