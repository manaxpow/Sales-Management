...
namespace blazor_web.Services.Cart
{
    public class CartService : ICartService
    {
        ...
        private async Task<int?> GetOrCreateActiveCartIdAsync()
        {
            ...
            try
            {
                var resp = await _http.GetFromJsonAsync<ApiResponse<List<CartResponse>>>(
                    $"carts/user/{userId}?status=active");
        ...
        public async Task<bool> AddItemAsync(blazor_web.Models.Product product, int quantity = 1)
        {
            try
            {
                ...
                var itemsResp =
                    await _http.GetFromJsonAsync<ApiResponse<List<CartItemResponse>>>($"cart-items?cartId={cartId}");
                if (itemsResp == null)
                {
                ...
                if (itemsResp.Success && itemsResp.Data != null)
                {
                    ...
                    if (existing != null)
                    {
                        ...
                        var updateReq = new CartItemUpdateRequest
                        {
                            ...
                            Quantity = newQuantity > existing.QuantityRemain ? existing.QuantityRemain : newQuantity,
                            Unit = existing.Unit
                        };
