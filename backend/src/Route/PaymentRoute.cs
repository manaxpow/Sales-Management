...
public static class PaymentRoute {
    public static IEndpointRouteBuilder MapPaymentEndPoint(this IEndpointRouteBuilder group) {
        ...
        // CREATE
        route.MapPost("/", async (Payments body, IValidator<Payments> validator, IPaymentService svc) => {
            ValidationResult result = await validator.ValidateAsync(body);
            if (!result.IsValid)
                return Results.BadRequest(result.Errors.Select(e => new { field = e.PropertyName, message = e.ErrorMessage }));
        ...
        // UPDATE
        route.MapPut("/{id:int}", async (int id, Payments body, IValidator<Payments> validator, IPaymentService svc) => {
            ValidationResult result = await validator.ValidateAsync(body);
            if (!result.IsValid)
                return Results.BadRequest(result.Errors.Select(e => new { field = e.PropertyName, message = e.ErrorMessage }));
